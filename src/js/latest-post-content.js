...
const getLatestPost = async () => {
  try {
    const investingNews = await DataSource.gettAllNews({
      keyword: 'investing',
    })
    const investingNewsData = investingNews['articles'].filter(
      (data) => !!isNull(data),
    )

    const latestPostContentContainer = document.getElementById(
    ...
    for (let i = 0; i < 4; i++) {
      latestPostContentContainer.innerHTML += `
        <article class="feature-1 w-full mb-16 md:mb-0 max-h-[300px]">
          <img src="${
            investingNewsData[i]['urlToImage']
          }" alt="education" class="w-full h-40 object-none image-source">
          <div class="px-1">
            <a href="${
              investingNewsData[i]['url']
            ...
            <div class="flex justify-between items-center mt-5 text-sm">
              <div class="overflow-hidden">
                <h4 class="author font-semibold w-[80%] text-ellipsis">${
                  investingNewsData[i]['author']
                }</h4>
                <div class="flex items-center mt-1">
                  <p class="date">${formatDate(
                    investingNewsData[i]['publishedAt'],
                  )}</p>
                  <div class="bg-[#06283D] rounded-full h-1 w-1 mx-2"></div>
                  <p class="source">${
                    investingNewsData[i]['source']['name']
                  }</p>
                </div>
              </div>
