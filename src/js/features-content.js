...
const getFeaturedPost = async () => {
  try {
    ...
    const healthNews = await DataSource.gettAllNews({
      keyword: 'health',
    })
    const healthNewsData = healthNews['articles'].find((data) => !!isNull(data))
    feature1.innerHTML = `
		   <img src="${
         healthNewsData['urlToImage']
       }" alt="health-image" class="w-full h-40 object-none  image-source ">
			<div class="px-1">
				<a href="${
          healthNewsData['url']
        }" target="_blank" rel="noopener nofollow" class="hover:underline title font-semibold block text-lg my-2 overflow-hidden h-14  text-ellipsis line-clamp-2">
    ...
    const techNews = await DataSource.gettAllNews({
      keyword: 'technology',
    })
      ...
      feature2.innerHTML += `
		   <article class="feature-2 h-[100px] md:h-[150px] flex">
					...
					<div class="flex flex-col overflow-hidden">
						<a href="${techNewsData[i]['url']}" target="_blank" rel="noopener nofollow"
    ...
    const artNews = await DataSource.gettAllNews({
      keyword: 'art',
    })
    ...
    for (let i = 0; i < 3; i++) {
      feature3.innerHTML += `
				<article class="feature-3 h-[100px] flex">
					<img src="${
            artNewsData[i]['urlToImage']
          }" alt="art-image" class="w-[120px] h-full object-cover mr-2">
					<div class="flex flex-col">
						<a href="${artNewsData[i]['url']}" target="_blank" rel="noopener nofollow"
							class="hover:underline title font-semibold text-sm h-10 overflow-hidden text-ellipsis line-clamp-2">
							${artNewsData[i]['title']}
						</a>
